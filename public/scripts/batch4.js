...
function resetLabelOpacity() {
    ...
    // Nav Right
    if (!not_possible && !possible && !incomplete_scene && !language_use && !unknown || position > 499)
...
function left() {
    ...
    nav_position.innerHTML = position.toString() + "/565";
    loadIndexVideo(index[position]);
    resetLabelOpacity();
}

function right() {
    if (position < 565) position++;
    if (position < index.length) {
        loadIndexVideo(index[position]);
        labels[position - 1] = getLabel();
        firebase.firestore().collection("batch4").doc(current_id.toString()).set({
            id: current_id,
            label: getLabel()
        })
        resetLabelButtons();
...
async function getVideo() {
    ...
    while (document_exists) {
        let docRef = firebase.firestore().collection("batch4").doc(storage_vids[rand_num].toString());
    ...
    const video = 'batch4/' + storage_vids[rand_num].toString() + '.mp4';
...
function loadIndexVideo(id) {
    current_id = id;
    const video = 'batch4/' + id.toString() + '.mp4';
    const storageRef = firebase.storage().ref();
    storageRef.child(video).getDownloadURL().then(function (url) {
        document.querySelector('#display-source').src = url;
        const display_video = document.querySelector('#display-video');
...
async function checkLabels() {
    const labs = firebase.firestore().collection("batch4").get().then(function (querySnapshot) {
        querySnapshot.forEach(function (doc) {
            labeled_vids[labeled_vids.length] = parseInt(doc.data().id);
            labels.push(doc.data().label);
            index.push(doc.data().id);
            position++;
        });
    });
    var storageRef = firebase.storage().ref("batch4");
    const stor = storageRef.listAll().then(function (result) {
        result.items.forEach(function (item) {
            vid = parseInt(item.location.path.replace("batch4/", "").replace(".mp4", ""));
