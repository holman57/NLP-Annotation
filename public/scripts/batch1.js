...
function left() {
    ...
    nav_position.innerHTML = position.toString() + "/544";
    loadIndexVideo(index[position]);
    resetLabelOpacity();
}

function right() {
    if (position < 544) position++;
    if (position < index.length) {
        loadIndexVideo(index[position]);
        labels[position - 1] = getLabel();
        firebase.firestore().collection("batch1").doc(current_id.toString()).set({
            id: current_id,
            label: getLabel()
        })
        resetLabelButtons();
...
async function getVideo() {
    ...
    while (document_exists) {
        let docRef = firebase.firestore().collection("batch1").doc(storage_vids[rand_num].toString());
    ...
    const video = 'batch1/' + storage_vids[rand_num].toString() + '.mp4';
...
function loadIndexVideo(id) {
    current_id = id;
    const video = 'batch1/' + id.toString() + '.mp4';
    const storageRef = firebase.storage().ref();
    storageRef.child(video).getDownloadURL().then(function (url) {
        document.querySelector('#display-source').src = url;
        const display_video = document.querySelector('#display-video');
...
async function checkLabels() {
    const labs = firebase.firestore().collection("batch1").get().then(function (querySnapshot) {
    ...
    var storageRef = firebase.storage().ref("batch1");
    const stor = storageRef.listAll().then(function (result) {
        result.items.forEach(function (item) {
            vid = parseInt(item.location.path.replace("batch1/", "").replace(".mp4", ""));
