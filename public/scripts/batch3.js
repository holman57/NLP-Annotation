...
function right() {
    ...
    if (position < index.length) {
        ...
        firebase.firestore().collection("batch3").doc(current_id.toString()).set({
            id: current_id,
            label: getLabel()
        })
        resetLabelButtons();
...
async function getVideo() {
    ...
    while (document_exists) {
        let docRef = firebase.firestore().collection("batch3").doc(storage_vids[rand_num].toString());
    ...
    const video = 'batch3/' + storage_vids[rand_num].toString() + '.mp4';
...
function loadIndexVideo(id) {
    current_id = id;
    const video = 'batch3/' + id.toString() + '.mp4';
    const storageRef = firebase.storage().ref();
    storageRef.child(video).getDownloadURL().then(function (url) {
        document.querySelector('#display-source').src = url;
        const display_video = document.querySelector('#display-video');
...
async function checkLabels() {
    const labs = firebase.firestore().collection("batch3").get().then(function (querySnapshot) {
    ...
    var storageRef = firebase.storage().ref("batch3");
    const stor = storageRef.listAll().then(function (result) {
        result.items.forEach(function (item) {
            vid = parseInt(item.location.path.replace("batch3/", "").replace(".mp4", ""));
